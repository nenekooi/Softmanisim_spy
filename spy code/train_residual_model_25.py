...
# --- 配置 ---
# !!! 将这里替换成你的数据文件的实际路径 !!!
DATA_FILE_PATH = 'D:/data/save_data/random_data_test9(u_new_4,cab=0.04,k=-20,a=1).xlsx'
MODEL_SAVE_DIR = 'D:/data/dl_model_0_random_data(u_new_4,cab=0.04,k=-20,a=1)' # 保存模型和 scaler 的目录
...
# 设置训练参数
EPOCHS = 300 # 最大训练轮数 (epoch)
BATCH_SIZE = 25 # 每批处理的样本数，可根据数据集大小和内存调整
...
def plot_history(history):
    ...
    # 保存图像
    plot_path = os.path.join(MODEL_SAVE_DIR, 'training_histor.png')
...
plt.grid(True)
scatter_path = os.path.join(MODEL_SAVE_DIR, 'prediction_scatter_Z.png')
