...
def calculate_curvatures_with_simple_stiffness(dl_segment, d, L0_seg, bending_stiffness_param, num_cables=3):
    """
    根据绳长变化量计算曲率 ux, uy，并引入简化的刚度影响。

    Args:
        dl_segment (np.array): 包含3根绳长变化量 [dl1, dl2, dl3] (单位: 米)。
        d (float): 缆绳到中心线的径向距离 (单位: 米)。
        L0_seg (float): 单段机器人的参考长度 (单位: 米)。 (当前模型未使用，但保留接口)
        bending_stiffness_param (float): 有效抗弯刚度参数 (无量纲，非负)。
                                         值越大表示越硬，曲率越小。
                                         值为 0 时，等同于原运动学模型。
                                         需要用户根据仿真与实际对比进行调试。
        num_cables (int): 缆绳数量，目前仅支持 3。
    ...
    if len(dl_segment) != num_cables:
        ...
        return ux, uy
    if num_cables != 3:
        print(f"错误: 此简化刚度模型目前仅支持3根绳索。")
        return 0.0, 0.0

    # --- 1. 计算基础的运动学曲率 (基于你的原公式) ---
    dl1, dl2, dl3 = dl_segment[0], dl_segment[1], dl_segment[2]
    ...
    if abs(denominator_ux) > 1e-9:
        ux_kinematic = (dl3 - dl2) / denominator_ux
    else:
        ux_kinematic = 0.0
        print("警告: 计算运动学 ux 时分母接近零。")

    # --- 2. 应用简化的刚度影响因子 ---
    # 确保刚度参数非负
    stiffness_param = max(0.0, bending_stiffness_param)

    # 计算刚度导致的曲率缩减因子 (Stiffness Reduction Factor)
    # stiffness_param越大，因子越小 (趋近于0)，曲率越小
    # stiffness_param为0时，因子为1，曲率不变
...
if __name__ == "__main__":
    ...
    # 1. 输入数据文件路径
    DATA_FILE_PATH = 'c:/Users/11647/SoftManiSim/spy code/circle2.xlsx' # <<< 确认 Excel 文件路径
    SHEET_NAME = 'Sheet1'
    # 2. 输出结果文件路径
    output_dir = os.path.dirname(DATA_FILE_PATH) if os.path.dirname(DATA_FILE_PATH) else '.'
 
    OUTPUT_RESULTS_PATH = os.path.join(output_dir, 'delete.xlsx')
    ...
    #有效抗弯刚度参数 (需要调试)
    EFFECTIVE_BENDING_STIFFNESS = 0.5  
    ...
    try:
        while simulation_running:
            ...
            ux, uy = calculate_curvatures_with_simple_stiffness(
                dl_segment, 
                cable_distance, 
                L0_seg, 
                EFFECTIVE_BENDING_STIFFNESS, # <<< 传入你定义的刚度参数
                num_cables
                )
