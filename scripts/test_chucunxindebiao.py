...
# 将结果可视化在 PyBullet 中 (机器人竖直向下 - 根据用户反馈调整旋转轴)，
# 同时记录每个输入绳长对应的末端 XYZ 坐标，并保存到新的 Excel 文件。
# ==============================================================================
...
try:
    ...
    if softmanisim_path not in sys.path:
     ...
     print("[警告] 无法自动确定项目根目录，请确保 visualizer 模块在 Python 路径中。")
...
try:
    ...
    print(f"[错误] 无法导入 ODE 类: {e}")
    print("[调试] 当前 sys.path:", sys.path)
    sys.exit("请检查 visualizer 模块的位置和 Python 路径 (PYTHONPATH) 设置。")
...
def calculate_orientation(point1, point2):
    ...
    if np.linalg.norm(diff) < 1e-6: return p.getQuaternionFromEuler([0,0,0]), [0,0,0]
...
def rotate_point_3d(point, rotation_angles):
    ...
    """根据欧拉角旋转一个3D点"""
    rx, ry, rz = rotation_angles; # ... (内部省略) ...

# --- 绳长变化量 -> 曲率 的转换函数 (!!! 需要用户根据实际模型验证或修改 !!!) ---
def calculate_curvatures_from_dl(dl_segment, d, L0_seg, num_cables=3):
    """根据绳长变化量计算曲率 ux, uy (简化示例)。"""
    ux = 0.0; uy = 0.0; # ... (内部省略，使用 dl 计算 ux, uy) ...
    if abs(d) < 1e-9: return 0.0, 0.0
    if len(dl_segment) != num_cables: return ux, uy
    if num_cables == 3:
        dl1, dl2, dl3 = dl_segment[0], dl_segment[1], dl_segment[2]
        gain = 1.0 / d # !!! 示例增益 !!!
        uy = gain * (dl1 - (dl2 + dl3) / 2.0)
        ux = gain * (math.sqrt(3.0) / 2.0) * (dl3 - dl2)
    else: print(f"错误: 当前示例仅为3根绳索实现了曲率计算。"); return 0.0, 0.0
    max_curvature = 50.0; ux = np.clip(ux, -max_curvature, max_curvature); uy = np.clip(uy, -max_curvature, max_curvature)
    return ux, uy
...
if __name__ == "__main__":
    ...
    OUTPUT_RESULTS_PATH = os.path.join(output_dir, 'Simulated_Data_1.xlsx') # <<< 修改输出文件名以区分

    # 3. 机器人物理参数
    num_cables = 3
    cable_distance = 7.5e-3 # 单位: 米
    initial_length = 0.15   # 单位: 米
    number_of_segment = 1
    ...
    absolute_lengths_mm = None; dl_sequence_m = None
    ...
    try:
        ...
        print(f"[成功] 已加载数据，共 {len(df_lengths)} 行。")
    ...
    if dl_sequence_m is None: print("[错误] 未能成功计算绳长变化量序列。"); sys.exit(1)
    ...
    p.resetDebugVisualizerCamera(
        cameraDistance=0.8, cameraYaw=45, cameraPitch=-20, cameraTargetPosition=[0, 0, 0.3]
    )
    print("[信息] 已设置相机视角。")
    ...
    base_pos = np.array([0, 0, 0.6]) # 抬高基座 Z 坐标
    # !!! 根据用户反馈，修改基座姿态: 绕 X 轴旋转 -90 度 !!!
    ...
    print(f"[设置] 机器人基座世界姿态 (Euler): {base_ori_euler} (绕X轴旋转-90度)")
    ...
    num_initial_bodies = len(my_robot_bodies) # 创建末端
    ...
    if len(positions0_local) >= 3 and num_initial_bodies == my_number_of_sphere:
        try:
            ...
            gripper_offset1 = [0, 0.01, 0]; pos1, _ = p.multiplyTransforms(pos_tip_world, ori_tip_world, gripper_offset1, [0,0,0,1])
            ...
            gripper_offset2 = [0,-0.01, 0]; pos2, _ = p.multiplyTransforms(pos_tip_world, ori_tip_world, gripper_offset2, [0,0,0,1])
    ...
    # --- 初始化用于存储结果的列表 ---
    results_data = { 'cblen1_mm': [], 'cblen2_mm': [], 'cblen3_mm': [], 'sim_X_m': [], 'sim_Y_m': [], 'sim_Z_m': [] }
    ...
    try:
        while simulation_running:
            ...
            pos_tip_world = np.array([np.nan, np.nan, np.nan]) # 初始化为 NaN
            if sol is not None and sol.shape[1] >= 3:
                ...
                for i in range(num_spheres_to_update):
                    ...
                    # !!! 使用正确的、已设为垂直的 base_pos 和 base_ori 进行变换 !!!
                    pos_world, ori_world = p.multiplyTransforms(base_pos, base_ori, pos_local, [0, 0, 0, 1])
                    try:
                ...
                if num_points_available >= 3 and num_bodies_total >= num_tip_bodies:
                    try:
                        ...
                        pos_tip_world = np.array(pos_tip_world_tuple) # 获取 (X, Y, Z)
                        ...
                        if len(my_robot_bodies) >= num_tip_bodies:
                         ...
                         if frame_count % 120 == 0: print(f"警告: 更新末端或记录位置时出错 - {e}") # 降低打印频率
                         pos_tip_world = np.array([np.nan, np.nan, np.nan]) # 出错时记录 NaN

            elif sol is None or sol.shape[1] < 3: # ODE 求解失败或点不够
                if frame_count % 120 == 0: print(f"警告: 第 {current_row_index} 帧无法获取有效形状({sol.shape if sol is not None else 'None'})，跳过位置记录。") # 降低打印频率
                pos_tip_world = np.array([np.nan, np.nan, np.nan]) # 记录 NaN
            ...
            results_data['sim_X_m'].append(pos_tip_world[0])
            results_data['sim_Y_m'].append(pos_tip_world[1])
            results_data['sim_Z_m'].append(pos_tip_world[2])
        ...
        if 'physicsClientId' in locals() and p.isConnected(physicsClientId): # 检查是否已连接
